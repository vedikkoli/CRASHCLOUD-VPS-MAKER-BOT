...
async def setup_container(container_id, status_msg, memory, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("ðŸŽ¨ Setting up HostForge customization...", ephemeral=True)
        else:
            await status_msg.edit(content="ðŸŽ¨ Setting up HostForge customization...")
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("âœ… HostForge VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="âœ… HostForge VPS setup completed successfully!")
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="ðŸ¤– HostForge VPS Bot Commands", color=discord.Color.blue())
...
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        status_msg = await ctx.send("ðŸš€ Creating HostForge VPS instance... This may take a few minutes.")
        ...
        await status_msg.edit(content="ðŸ”§ Container created. Setting up HostForge environment...")
        await asyncio.sleep(5)
        ...
        try:
            embed = discord.Embed(title="ðŸŽ‰ HostForge VPS Creation Successful", color=discord.Color.green())
            ...
            embed.add_field(name="ðŸ”Œ Direct SSH", value=f"```ssh root@{bot.public_ip} -p {external_port}```", inline=False)
            embed.add_field(name="â„¹ï¸ Note", value="This is a HostForge VPS instance. You can install and configure additional packages as needed.", inline=False)
            
            await owner.send(embed=embed)
            await status_msg.edit(content=f"âœ… HostForge VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"âœ… HostForge VPS {vps_id} has been deleted successfully!")
    except Exception as e:
...
""", inline=False)
        ...
        await ctx.send("âœ… Connection details sent to your DMs! Use the Tmate command to connect to your HostForge VPS.", ephemeral=True)
...
async def run_command(ctx, vps_id: str, command: str):
    ...
    try:
        ...
        try:
            ...
            if success:
                await ctx.send(f"âœ… Command executed successfully:\n```{output}```", ephemeral=True)
            else:
                await ctx.send(f"âŒ Command failed:\n```{output}```", ephemeral=True)
        except Exception as e:
            await ctx.send(f"âŒ Error running command: {str(e)}", ephemeral=True)
    except Exception as e:
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("ðŸ”„ Reinstalling HostForge bot... This may take a few minutes.")
...
class VPSManagementView(ui.View):
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.followup.send("âœ… HostForge VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("âœ… HostForge VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.followup.send("âœ… HostForge VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("ðŸ”„ Reinstalling HostForge VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"âŒ Error reinstalling HostForge VPS {self.vps_id}: {str(e)}")
                except:
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"âœ… HostForge VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
